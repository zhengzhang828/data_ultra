...
preprocess_path_ex = os.path.join(_dir,'example')
img_train_path_ex = os.path.join(preprocess_path_ex,'imgs_train.npy')
img_train_mask_path_ex = os.path.join(preprocess_path_ex,'imgs_mask_train.npy')
img_train_patients_ex = os.path.join(preprocess_path_ex, 'imgs_patient.npy')
...
def copyimage():
    example_path = os.path.join(data_path,'example')
    print ('example_path: ',example_path)
    copy_path = os.path.join(data_path,'train')
    images_path = os.listdir(copy_path)
    sub_images = filter(lambda s: s.startswith('1'), images_path)
    images = filter(lambda image: 'mask' not in image, sub_images)
    total = len(images)
    ...
    for image_name in images:
        ...
        #image_mask_name_ex = image_name.split('.')[0] + '_mask.jpg'

        img = cv2.imread(os.path.join(copy_path,image_name), cv2.IMREAD_GRAYSCALE)
        img_mask = cv2.imread(os.path.join(copy_path,image_mask_name), cv2.IMREAD_GRAYSCALE)
        #print ('image_name:',image_name)
        #cv2.imwrite(os.path.join(example_path, image_mask_name_ex),img_mask)
        ...
        i += 1
    print('Loading done.')
    np.save(img_train_patients_ex, img_patients)
    np.save(img_train_path_ex, imgs)
    np.save(img_train_mask_path_ex, imgs_mask)
